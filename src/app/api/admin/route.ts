...
export async function POST(req: NextRequest) {
    try {
        ...
        if (action === 'create_trial') {
            ...
            // 3. 사용자 즉시 관리자로 등록 (최대한 모든 방법 동원)
            // [보안] 슈퍼어드민은 체험판을 만들더라도 주 교회 소속을 유지하도록 보호
            const HARDCODED_ADMINS = (process.env.NEXT_PUBLIC_ADMIN_EMAIL || "pastorbaek@kakao.com,kakao_4761026797@kakao.somy-qt.local").toLowerCase().split(',').map(e => e.trim());
            ...
            if (!isAdminEmail) {
                ...
                try {
                    ...
                    const dummyMembers = [];
                    for (let i = 0; i < 30; i++) {
                        const dName = lastNames[Math.floor(Math.random() * lastNames.length)] + firstNames[Math.floor(Math.random() * firstNames.length)] + (i + 1);
                        dummyMembers.push({
                            church_id: trialChurchId,
                            full_name: dName,
