...
export default function FamilyTree({
    ...
    // 카드 렌더러
    const renderNodeCard = (rel: any, isSelf = false) => {
        return (
            <div 
                key={rel.id} 
                style={{
                    background: isSelf ? 'linear-gradient(135deg, #FFFDF3 0%, #FFF8D5 100%)' : 'white',
                    border: isSelf ? '2px solid #D4AF37' : '1px solid #EAEAEA',
                    borderRadius: '16px',
                    padding: '10px 14px',
                    width: '125px',
                ...
                title={isSelf ? '현재 선택된 성도' : '이 성도의 상세/가계도로 이동'}
            >
                {/* 사진 */}
                ...
                <div style={{ 
                    fontSize: '9px', 
                    background: isSelf ? '#333' : '#F4F4F4', 
                    color: isSelf ? 'white' : '#666', 
                    padding: '2px 6px', 
                    borderRadius: '6px', 
                    fontWeight: 700 
                }}>
                    {isSelf ? '본인 ⭐' : (relLabels[rel.relationship_type] || rel.relationship_type)}
                </div>

                {/* 관리 액션 버튼 (본인이 아닐 때만 노출) */}
                {!isSelf && (
                    <div 
                        style={{ position: 'absolute', top: '4px', right: '4px', display: 'flex', gap: '2px' }}
                        onClick={(e) => e.stopPropagation()} // 카드 클릭 이동 방지
                    >
                        {/* 편집 셀렉터 */}
                        <select
                        ...
                        <span style={{ fontSize: '10px', width: '16px', height: '16px', display: 'flex', alignItems: 'center', justifyContent: 'center', borderRadius: '4px', background: '#F5F5F5', cursor: 'pointer', border: '1px solid #DDD' }} title="관계 유형 변경">⚙️</span>
                ...
                <div style={{
                    ...
                    <div>
                        ...
                        <p style={{ margin: '2px 0 0 0', fontSize: '11px', color: '#888' }}>
                            노드를 클릭하면 해당 성도의 가계도로 즉시 전환 이동합니다.
                        </p>
                    </div>
                ...
                <div style={{ flex: 1, overflowY: 'auto', padding: '24px', display: 'flex', flexDirection: 'column', gap: '28px', background: '#FAF9F6' }}>
                    {isLoading ? (
                        ...
                        <>
                            ...
                            {/* Level 2: 부모 */}
                            <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '6px' }}>
                                <div style={{ fontSize: '10px', color: '#AAA', fontWeight: 700, letterSpacing: '1px', textTransform: 'uppercase', marginBottom: '2px' }}>PARENTS</div>
                                <div style={{ display: 'flex', gap: '12px', justifyContent: 'center', flexWrap: 'wrap' }}>
                                    {parents.length === 0 ? (
                                        <div 
                                            onClick={() => {
