...
// src/app/api/admin/upload-avatar/route.ts
var route_exports = {};
...
async function POST(req) {
  try {
    ...
    if (!file || !userId) {
      return import_server.NextResponse.json({ error: "Missing file or user_id" }, { status: 400 });
    }
    ...
    const { data: uploadData, error: uploadError } = await supabaseAdmin.storage.from("member-photos").upload(filePath, file, {
      contentType: file.type,
      upsert: true
    });
    if (uploadError) throw uploadError;
    const { data: { publicUrl } } = supabaseAdmin.storage.from("member-photos").getPublicUrl(filePath);
    const { error: updateError } = await supabaseAdmin.from("profiles").update({ avatar_url: publicUrl }).eq("id", userId);
    if (updateError) throw updateError;
    return import_server.NextResponse.json({ url: publicUrl });
  } catch (err) {
